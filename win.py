...
class ListPage():
    ...
    def create(self):
        ...
        def dir_select():
            ...
            DefaultDir = PureWindowsPath('D:\\')
            options = {}
            options['parent'] = self.page
            SelectedDir = PureWindowsPath(tk.filedialog.askdirectory(title='选择文件', initialdir=(os.path.expanduser(DefaultDir)), **options))
        ...
        def confirm():
            ...
            for subdir in subdirs:
                subdir_addr = PureWindowsPath(os.path.join(maindir, subdir))
            ...
            for file in files:
                file_addr = PureWindowsPath(os.path.join(maindir, file))
...
class Md5Page():
    ...
    def dir_select(self):
        ...
        options = {}
        options['parent'] = self.page
        SelectedDir = PureWindowsPath(tk.filedialog.askdirectory(title='选择路径', initialdir=(os.path.expanduser(DefaultDir)), **options))
        self.DirEntry.insert(0, SelectedDir)
    ...
    def text_save(self):
        f = PureWindowsPath(tkinter.filedialog.asksaveasfile(title = '选择保存路径', mode='w', filetypes=(("text files", "*.txt"), ("all files", "*.*"))))
        if f is None:
            return
